...
class ProjectManager(RoleZero):
    ...
    max_react_loop: int = (
        1  # FIXME: Read and edit files requires more steps, consider later
    )
    ...
    async def _run_special_command(self, cmd) -> str:
        """Override to handle special commands for ProjectManager role."""
        if cmd["command_name"] == "Terminal.run_command":
            return "ProjectManager does not have the capability to run terminal commands directly. Please assign this task to Engineer2 (Alex), DataAnalyst (David), or Architect (Bob) who have the appropriate technical capabilities to execute code and run commands."
