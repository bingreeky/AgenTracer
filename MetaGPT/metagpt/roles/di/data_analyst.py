...
class DataAnalyst(RoleZero):
    ...
    goal: str = (
        "Take on any data-related tasks, such as data analysis, machine learning, deep learning, web browsing, web scraping, web searching, terminal operation, document QA & analysis, etc."
    )
    ...
    write_code: WriteAnalysisCode = Field(
        default_factory=WriteAnalysisCode, exclude=True
    )
    ...
    async def write_and_exec_code(self, instruction: str = ""):
        ...
        while not success and counter < 3:
            ...
            self.rc.working_memory.add(
                Message(content=code, role="assistant", cause_by=WriteAnalysisCode)
            )
        ...
        if self.log_step:
            self.log_step(
                f"{self.profile} write and exec code:\n{code}\nResult:\n{result}",
                role="assistant",
                name=self.profile,
            )
        return output
    ...
    async def _run_special_command(self, cmd) -> str:
        ...
        if command_output != "":
            if self.log_step:
                self.log_step(
                    f"Terminal output: {command_output}",
                    role="Terminal",
                    name=self.profile,
                )
