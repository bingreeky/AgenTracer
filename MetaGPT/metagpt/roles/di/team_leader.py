...
class TeamLeader(RoleZero):
    ...
    def publish_team_message(self, content: str, send_to: str):
        ...
        if self.log_step:
            self.log_step(
                f"{self.profile} sends to {send_to}: {content}",
                role="assistant",
                name=self.profile,
            )
    ...
    async def _run_special_command(self, cmd) -> str:
        """Override to handle special commands for TeamLeader role."""
        if cmd["command_name"] == "Terminal.run_command":
            return "TeamLeader does not have the capability to run terminal commands directly. Please assign this task to Engineer2 (Alex), DataAnalyst (David), or Architect (Bob) who have the appropriate technical capabilities to execute code and run commands."
