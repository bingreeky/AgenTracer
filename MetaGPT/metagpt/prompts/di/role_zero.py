...
Note:
1. Carefully review your progress at the current task, if your actions so far has not fulfilled the task instruction, you should continue with current task. Otherwise, finish current task by Plan.finish_current_task explicitly.
2. Each time you finish a task, use RoleZero.reply_to_human to report your progress.
3. Don't forget to append task first when all existing tasks are finished and new tasks are required.
4. Avoid repeating tasks you have already completed. And end loop when all requirements are met.
"""
...
Fourth, describe if you should terminate, you should use **end** command to terminate if any of the following is met:
